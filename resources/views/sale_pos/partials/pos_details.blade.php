<div class="row">
	<div class="col-sm-12">
		<div class="panel panel-default">
			<div class="panel-body bg-gray disabled" style="margin-bottom: 0!important">
					...
					<tr>
						...
						<td class="col-md-2">
							...
							@if(isset($transaction))
								<input type="hidden" name="discount_amount" id="discount_amount" value='{{$transaction->discount_amount}}' data-default='{{$transaction->discount_amount}}'>
							@else
								<input type="hidden" name="discount_amount" id="discount_amount" data-default="0" value="0">
							@endif
						...
						<td class="col-md-2">
							...
							<b>@lang('sale.bullet')(-): @show_tooltip(__('tooltip.sale_bullet'))</b> 
							<br/>
							<i class="fa fa-pencil-square-o cursor-pointer" id="pos-edit-bullet" title="@lang('sale.edit_bullet')" aria-hidden="true" data-toggle="modal" data-target="#posEditBulletModal"></i>
							<span id="total_bullet">0</span>

							<input type="hidden" name="points" id="bullet_amount" value="@if(empty($edit)) {{@num_format($business_details->default_sales_bullet)}} @else {{@num_format($transaction->points)}} @endif" data-default="{{$business_details->default_sales_bullet}}">
						...
						<td class="col-md-1">
							...
							<b>@lang('sale.delivery')(+):</b> 
							<br/>
							@if(empty($delivery))
							<span id="delivery_amount_span"> 0 </span>
							<input type="hidden" name="delivery_amount" 
								id="delivery_amount" value= 0>
							<input type="hidden" name="delivery_tax" 
								id="delivery_tax" value= 0 >
							<input type="hidden" name="delivery_uid" 
								id="delivery_uid" value= 0 >
							@else
							<span id="delivery_amount_span"> {{$delivery->delivery_price}} </span>
							<input type="hidden" name="delivery_amount" 
								id="delivery_amount" value= {{$delivery->delivery_price}}>
							<input type="hidden" name="delivery_tax" 
								id="delivery_tax" value= {{$delivery->delivery_tax}} >
							<input type="hidden" name="delivery_uid" 
								id="delivery_uid" value= {{$delivery->delivery_uid}} >
					...
					<tr>
						...
						<div class="">
							...
							<div class="col-sm-10 col-no-padding">
                                ...
                                <div class="col-sm-4 col-2px-padding">
                                    <button type="button"
                                            class="btn bg-maroon btn-block btn-flat btn-lg no-print pos-express-btn pos-card-machine"
                                            data-pay_method="card_machine"
                                            title="@lang('lang_v1.tooltip_express_checkout_card_machine')" >
                                        <div class="text-center">
                                            <i class="fa fa-check" aria-hidden="true"></i>
                                            <b>@lang('lang_v1.express_checkout_card_machine')</b>
                                        </div>
                                    </button>
                                </div>
...
@if(isset($transaction))
	@include('sale_pos.partials.edit_bullet_modal', ['sales_bullet' => $transaction->bullet_amount])
@else
	@include('sale_pos.partials.edit_bullet_modal', ['sales_bullet' => $business_details->default_sales_bullet])
@endif
