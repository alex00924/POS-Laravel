    ...
    */
    ...
    public function __construct(BusinessUtil $businessUtil, RestaurantUtil $restaurantUtil, ModuleUtil $moduleUtil)
    {
        ...
        $this->avlble_modules = ['tables' => 
                ...
                'delivery' => [ 'name' => __('restaurant.delivery')]
            ];
        $this->theme_colors = [
            'blue' => 'Blue',
            'black' => 'Black',
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only(['name', 'start_date', 'currency_id', 'time_zone', 'business_type']);
    ...
    public function getBusinessSettings()
    {
        ...
        $locations = BusinessLocation::where('business_id', $business_id)->select(['id']);
        ...
        $business_location = BusinessLocation::where('id', $location)->first();
        $business['default_sales_discount'] = $business_location['default_sales_discount'];
        $business['sell_price_tax'] = $business_location['sell_price_tax'];
        $business['sales_cmsn_agnt'] = $business_location['sales_cmsn_agnt'];
        $business['item_addition_method'] = $business_location['item_addition_method'];

        $business['braintree_env'] = $business_location['braintree_env'] == NULL ? 'none' : $business_location['braintree_env'];
        $business['minimum_amount'] = $business_location['minimum_amount'] == NULL ? 500 : $business_location['minimum_amount'];
        $business['braintree_merchant_account_id'] = $business_location['braintree_merchant_account_id'];
        $business['braintree_merchant_id'] = $business_location['braintree_merchant_id'];
        $business['braintree_public_key'] = $business_location['braintree_public_key'];
        $business['braintree_private_key'] = $business_location['braintree_private_key'];
        $business['is_delivery'] = Subscription::can_delivery($business_id);
        ...
        $delivery = Delivery::where('business_loc_id',$location)->first();    
        if( empty($delivery ) )
        {
            $delivery = array('business_id'=>$business_id, 'business_loc_id'=>$location, 'areas'=>'[]', 'is_all'=>'0', 'product_ids'=>'[]', 'charge'=>'0', 'tax_type'=>'inclusive', 'tax'=>'', 'is_minimum'=>'0', 'minimum'=>'0', 'email'=>'', 'enabled'=>'0');
        ...
        return view('business.settings', compact('business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'delivery', 'locations', 'location'));
    }
    ...
    * @param  \Illuminate\Http\Request  $request
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            //     ]
            // );
            if (!auth()->user()->can('superadmin') && $request->braintree_env !== 'none') {
                # Brain tree initialization
                Braintree_Configuration::environment($request->braintree_env);
                Braintree_Configuration::merchantId($request->braintree_merchant_id);
                Braintree_Configuration::publicKey($request->braintree_public_key);
                Braintree_Configuration::privateKey($request->braintree_private_key);
                $clientToken = ClientToken::generate();

                $merchantAccount = Braintree\MerchantAccount::find($request->braintree_merchant_account_id);
            }

            
            $business_loc_id = $request->only('business_location_id')['business_location_id'];
            
            $business_details = $request->only(['name', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 
                'default_profit_percent', 'sku_prefix', 'time_zone', 'fy_start_month', 'default_sales_tax',
                'accounting_method', 'transaction_edit_days', 'currency_symbol_placement', 'on_product_expiry',
                'stop_selling_before', 'default_unit', 'expiry_type', 'date_format', 'time_format', 'ref_no_prefixes', 'theme_color']);
            ...
            //Default Sale Module
            $business_location =BusinessLocation::where('id', $business_loc_id)->first();
            $business_loc_detail = $request->only(['item_addition_method', 'default_sales_discount', 'sell_price_tax','sales_cmsn_agnt', 'minimum_amount',
                                'braintree_env', 'braintree_merchant_account_id', 'braintree_merchant_id', 'braintree_public_key', 'braintree_private_key']);
            if( !is_null($business_loc_detail))
            {
                $business_location->fill($business_loc_detail);
            ...
            if( $is_delivery )
            {
                $delivery = Delivery::where('business_loc_id', $business_loc_id)->first();
                $delivery_inputs=$request->only(['enable_delivery', 'delivery_charge', 'delivery_tax', 'delivery_tax_type', 'delivery_email', 'is_minimum', 'delivery_minimum', 'is_all', 'delivery_products', 'delivery_areas']);
                $delivery_details = [];
                $delivery_details['business_id'] = $business_id;
                $delivery_details['charge'] = $delivery_inputs['delivery_charge'];
                $delivery_details['tax'] = $delivery_inputs['delivery_tax'];
                $delivery_details['tax_type'] = $delivery_inputs['delivery_tax_type'];
                $delivery_details['email'] = $delivery_inputs['delivery_email'];
                $delivery_details['is_minimum'] = !empty($delivery_inputs['is_minimum']) &&  $delivery_inputs['is_minimum'] == 1 ? 1 : 0;
                $delivery_details['minimum'] = !empty($delivery_inputs['delivery_minimum']) ? $delivery_inputs['delivery_minimum'] : 0;
                $delivery_details['is_all'] = !empty($delivery_inputs['is_all']) &&  $delivery_inputs['is_all'] == 1 ? 1 : 0;
                $delivery_details['product_ids'] = $delivery_inputs['delivery_products'];
                $delivery_details['areas'] = $delivery_inputs['delivery_areas'];
                $delivery_details['enabled'] = (!empty($enabled_modules) && in_array('delivery', $enabled_modules)) ? 1 : 0; 
                $delivery->fill($delivery_details);
                $delivery->save();
                
            }

            return redirect('business/settings?business_location='.$business_loc_id)->with('status', $output);
        } catch (\Exception $e) {
            \Log::emergency("File:" . $e->getFile(). "Line:" . $e->getLine(). "Message:" . $e->getMessage());
            if ($e->getMessage() !== '')
                $output = ['success' => 0,
                ...
                $output = ['success' => 0,
                    'msg' => __('messages.braintree_error')
                ];
    ...
    public function getLocInfo($location_id)
    {
        $details = BusinessLocation::select('sell_price_tax', 'default_sales_discount', 'item_addition_method')
                                    ->where('id', $location_id)->first();
        $selling_price_group = SellingPriceGroup::forDropdown($location_id);
        $details['selling_price_group'] = $selling_price_group;
        return $details;
    }
