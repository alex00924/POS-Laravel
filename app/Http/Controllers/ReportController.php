...
class ReportController extends Controller
{
    ...
    public function getProfitLoss(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $data['total_bullet'] = !empty($sell_details['total_bullet']) ? $sell_details['total_bullet'] : 0;

            $data['total_delivery'] = !empty($sell_details['total_delivery']) ? $sell_details['total_delivery'] : 0;
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($products)
                ...
                ->editColumn('unit_price', function ($row) use($allowed_selling_price_group, $location_id) {
                    $html = '';
                    ...
                    if($allowed_selling_price_group){
                        $html .= ' <button type="button" class="btn btn-primary btn-xs btn-modal" data-container=".view_modal" data-href="/products/view-product-group-price/' . $row->DT_RowId . '/' . $location_id . '">' . __('lang_v1.view_group_prices') . '</button>';
                    }
    ...
    public function sellPaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                 ->editColumn('invoice_no', function ($row) {
                    return '<a data-href="' . action('SellController@show', [$row->transaction_id])
                            . '" href="#" data-container=".view_modal" class="btn-modal" data-uid="'.$row->uid.'">' . $row->invoice_no . '</a>';
                 })
                ->editColumn('paid_on', '{{@format_date($paid_on)}}')
                ->editColumn('method', function ($row) {
                    $method = __('lang_v1.' . $row->method);
