...
<div class="modal-body">
    ...
    <div class="row">
      ...
      <div class="col-sm-3">
          @if($sell->payment_lines[0]->method == "mobile")
              <b>{{ __('sale.mobile_pay') }}:</b> {{ $sell->payment_lines[0]->uid }}<br>
          @elseif($sell->payment_lines[0]->method == "card_machine")
              <b>{{ __('sale.receipt_no') }}:</b> {{ $sell->payment_lines[0]->receipt_no }}<br>
          @endif
          @if(!is_null($sell->contact))
      ...
      @if(!is_null($delivery))
      <div class="col-sm-3">
        <b>{{ __('delivery.user_name') }}:</b>  {{ $delivery->user_name }}<br>
        <b>{{ __('delivery.user_phone') }}:</b> {{ $delivery->user_phone }}<br>
        <b>{{ __('delivery.user_email') }}:</b> {{ $delivery->user_email }}
      </div>
      <div class="col-sm-3">
        <b>{{ __('delivery.area') }}:</b> {{ $delivery->order_area }}<br>
        <b>{{ __('delivery.area_address') }}:</b> {{ $delivery->order_area_address }}
      </div>
      @endif
    </div>
    <br>
        ...
        <div class="table-responsive">
          <table class="table bg-gray">
            ...
            <tr>
              ...
              <td><b>(+)</b></td>
              <td><span class="display_currency pull-right" data-currency_symbol="true">@if(!is_null($delivery)) {{ $delivery->delivery_price}}  @else{{ 0 }}@endif</span></td>
            </tr>
            <tr>
              <th>{{ __('delivery.delivery_tax') }}: </th>
              <td><b>(+)</b></td>
              <td><span class="display_currency pull-right" data-currency_symbol="true">@if(!is_null($delivery)) {{ $delivery->delivery_tax}}  @else{{ 0 }}@endif</span></td>
            </tr>
            ...
            <tr>
              <th>{{ __('sale.total_bullet') }}:</th>
              <td></td>
              <td><span class="pull-right">{{ $sell->points }}</span></td>
            </tr>
            <tr>
