...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Auth()->user()->can('superadmin')) {
            $requested_redeem = RedeemPoints::where('redeem_result', '0')
                ->orderBy('updated_at', 'DESC')
                ->select('id', 'business_loc_id', 'points')
                ->take(3)
                ->get();
            foreach ($requested_redeem as $redeem) {
                $redeem->location->select('name');
            }
        }

        return view('home.index', compact('date_filters', 'sells_chart_1', 'sells_chart_2', 'requested_redeem'));
    }
    ...
    {
        if (request()->ajax()) {
            /** if superadmin's  business*/
            if (Auth()->user()->username == env('ADMINISTRATOR_USERNAMES')) {

                return ['points' => -1, 'pending_points' => 0];
            } else {
                $business_id = request()->session()->get('user.business_id');

                $business_points = Business::where('id', $business_id)->select('points')->first();
                $pending_redeem_points = RedeemPoints::where('business_id', $business_id)
                    ->where('redeem_result', 0)
                    ->sum('points');

                return ['points' => $business_points->points, 'pending_points' => $pending_redeem_points];
            }
        }
    }
