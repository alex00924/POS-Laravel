        ...
        if(request()->ajax()){
           ...
           try{
                ...
                if($subscriptions->status == 'waiting' && $subscriptions->paid_via == 'offline' && empty($subscriptions->start_date && $input['status'] == 'approved')){
                    ...
                    $subscriptions->start_date = $start_date->toDateString();

                    //Calculate end date
                    $package = $subscriptions->package;
                    if($package->interval == 'days'){
                        $subscriptions->end_date = $start_date->addDays($package->interval_count)->toDateString();
                    } elseif($package->interval == 'months'){
                        $subscriptions->end_date = $start_date->addMonths($package->interval_count)->toDateString();
                    } elseif($package->interval == 'years'){
                        $subscriptions->end_date = $start_date->addYears($package->interval_count)->toDateString();
                    }
                    
                    $subscriptions->trial_end_date = $start_date->addDays($package->trial_days);
                }
