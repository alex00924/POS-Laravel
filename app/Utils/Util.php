...
class Util
{
    ...
    {
        $payment_types = ['cash' => __('lang_v1.cash'), 'mobile' => __('lang_v1.mobile') , 'card' => __('lang_v1.card'), 'card_machine' => __('lang_v1.card_machine'), 'cheque' => __('lang_v1.cheque'), 'bank_transfer' => __('lang_v1.bank_transfer'), 'other' => __('lang_v1.other')];
    ...
    public function disableDelivery($package_id)
    {
        $businessArr = Business::leftJoin('subscriptions','business.id','=','subscriptions.business_id')
                                ->where('subscriptions.package_id', $package_id)
                                ->groupBy('subscriptions.business_id')
                                ->get(['business.enabled_modules', 'business.id'])->toArray();
        foreach($businessArr as $business){
            $modules = $business['enabled_modules'];
            if( is_null($modules) )
                continue;
            $index = array_search('delivery', $modules);
            if($index !== false){
                unset($modules[$index]);
                Business::find($business['id'])->update(array('enabled_modules' => $modules));
            }
        }
    }
}
